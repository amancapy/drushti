...
def to_docx_subdir(args):
    ...
    for i in tqdm.tqdm(range(start, end), desc=f"{subdir}"):
...
def to_docx():
    ...
    for i in range(0, n_subdirs, 8):
        ...
        a = time.time()
        print(f"clipping subdirs {i} through {i+7} of {n_subdirs}.")
...
def to_text_subdir(args):
    ...
    for j in tqdm.tqdm(range(start, end), desc=f"{subdir}"):
...
def to_text_and_png():
    ...
    for i in range(0, n_docxs // 100, 8):
        ...
        a = time.time()
        print(f"converting subdirs {i} through {i+7} of {n_docxs // 100}.")
        sp.call(clip_cmd, shell=True, stdout=null)
        print(f"This batch took {time.time() - a} seconds.")
